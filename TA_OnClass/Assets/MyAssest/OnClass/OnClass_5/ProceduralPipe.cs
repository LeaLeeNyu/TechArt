...
public class ProceduralPipe : MonoBehaviour
{
    ...
    void Update()
    {
        for (int i = 0; i < segments; i++)
        {
            ...
            //Try to manipulate this paremeter to create different effect
        ...
        if (length <= 0)
        {
            ...
            while (currentDirection == currentCornerRotation * currentDirection)
            {
                PickNewRandomRotation();
            }
    ...
    void PickNewRandomRotation()
    {
        ...
        switch (direction)
        {
            //Bug: the twist will appear when the pipeline rotate along the direction
